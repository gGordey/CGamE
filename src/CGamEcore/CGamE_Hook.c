...
CGE_Object_id CGE_CreateHook (CGE_Context *Context, CGE_Hook_Callback_t Callback, CGE_Hook_type type) {
    CGE_Object_id hook = CGE_CreateObject(Context, OBJ_TYPE_HOOK, 0);
...
void CGE_DeattachHook (CGE_Context *Context, CGE_Object_id hook) {
    CGE_AttachHook (Context, &CGE_NullCallback, HOOK_DEATTACHED, hook);
}
...
void CGE_ActivateHookUpdate (CGE_Context *Context) {
    for (CGE_Object_id i = 0; i < Context->Objects.size; ++i) {
        ...
        if (obj->objId == UINT32_MAX ||
            obj->type != OBJ_TYPE_HOOK) {
                continue;
        }
            
        if (((CGE_Hook*)obj->data)->type == HOOK_UPDATE) {
            ((CGE_Hook*)obj->data)->Callback(Context, 0);
        }
    }
}

void CGE_NullCallback (CGE_Context* Context, int64_t zero) {}